...
def process_file():
  ...
  try:
    ...
    farmer_col = next(
        (c for c in df.columns if "farmer name" in c.lower()), None
    )
    father_col = next(
        (c for c in df.columns if "father name" in c.lower()), None
    )
    ...
    if not (khata_col and booking_col and crop_col and area_col):
      return (
          jsonify({
              "error": (
                  "Required columns (Khatha, Booking ID, Crop Name, Area) not"
                  " found."
              )
          }),
          400,
